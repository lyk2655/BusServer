...
public class BusController {
	...
	public String queryLine(HttpServletRequest request) {
		...
		logger.info("queryLine:[" + line + "]:" + "bigin time[" + t1 + "]-end time [" + t2 + "] time cost:["
	...
	public String queryBus(HttpServletRequest request) {
		...
		request.getSession().setAttribute("data", JSON.toJSONString(res));
		String t2 = new DateUtil().getTm();
		logger.info("queryBus:[" + line + "]:" + "bigin time[" + t1 + "]-end time [" + t2 + "] time cost:["
				+ String.valueOf((Integer.parseInt(t2.substring(4)) - Integer.parseInt(t1.substring(4))) / 100)
				+ "ms]");
		return "json";
	}
	...
	public String queryClosestStation(HttpServletRequest request) {
		...
		request.getSession().setAttribute("data", JSON.toJSONString(res));
		String t2 = new DateUtil().getTm();
		logger.info("queryClosestStation:bigin time[" + t1 + "]-end time [" + t2 + "] time cost:["
				+ String.valueOf((Integer.parseInt(t2.substring(4)) - Integer.parseInt(t1.substring(4))) / 100)
				+ "ms]");
		return "json";
	}
